...
export async function generateInvoicePDF(data) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const descripcionWidth = 280;
            const descripcionLineHeight = 12;
            const descripcionHeight = doc.heightOfString(descripcionTexto, { width: descripcionWidth, lineGap: 2 });
            const itemHeight = Math.max(30, descripcionHeight + 10);
            
            doc.fillColor(colors.black).font('Helvetica').text('1.00', 45, tableY + 30);
            doc.text(descripcionTexto, 100, tableY + 30, { width: descripcionWidth, lineGap: 2 });
